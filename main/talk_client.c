...
{
    if (s_talk.response_overflow) return;

    if (s_talk.response_length + 1 >= SPARKY_TALK_RESPONSE_MAX_BYTES) {
        s_talk.response_overflow = true;
        ESP_LOGW(TAG, "Response metadata exceeds %u bytes; truncating",
...
/*
 * The response includes a potentially large base64 "audio" string. Keep its
 * JSON delimiters but discard its contents, turning it into "audio":"".
 * This preserves valid JSON for cJSON while retaining only response metadata.
 */
static void collect_response_metadata(const char *data, size_t length)
{
    for (size_t i = 0; i < length; ++i) {
        ...
        case RESPONSE_SCAN_STRING:
            ...
            if (s_talk.string_escape) {
                ...
                s_talk.response_scan_state =
                    ...
                    RESPONSE_SCAN_AUDIO_COLON : RESPONSE_SCAN_NORMAL;
            } else if (s_talk.string_token_length < sizeof(s_talk.string_token) - 1) {
                s_talk.string_token[s_talk.string_token_length++] = byte;
            }
            break;
        ...
        case RESPONSE_SCAN_AUDIO_COLON:
            append_response_byte(byte);
            if (byte == ':') {
                s_talk.response_scan_state = RESPONSE_SCAN_AUDIO_VALUE;
        ...
        case RESPONSE_SCAN_AUDIO_VALUE:
            append_response_byte(byte);
            if (byte == '"') {
                s_talk.response_scan_state = RESPONSE_SCAN_AUDIO_STRING;
                s_talk.string_escape = false;
            } else if (byte != ' ' && byte != '\t' && byte != '\r' && byte != '\n') {
                s_talk.response_scan_state = RESPONSE_SCAN_NORMAL;
            }
...
static esp_err_t http_event(esp_http_client_event_t *event)
{
    if (event->event_id == HTTP_EVENT_ON_DATA && event->data_len > 0) {
        collect_response_metadata(event->data, (size_t)event->data_len);
    }
    return ESP_OK;
}
...
static void log_field(cJSON *root, const char *name)
{
    ...
    if (cJSON_IsString(item) && item->valuestring != NULL) {
        ESP_LOGI(TAG, "  %s: %s", name, item->valuestring);
    } else if (cJSON_IsNumber(item)) {
        ESP_LOGI(TAG, "  %s: %.3f", name, item->valuedouble);
    } else {
        ESP_LOGW(TAG, "  %s: <missing>", name);
    }
}
...
static void log_response(void)
{
    if (s_talk.response_overflow) {
        ESP_LOGE(TAG, "Response metadata is too large to parse");
        return;
    }

    cJSON *root = cJSON_Parse(s_talk.response);
    if (root == NULL) {
        ESP_LOGE(TAG, "Malformed JSON response");
        return;
    }
...
static bool write_all(esp_http_client_handle_t client, const char *data, size_t length)
{
    while (length > 0) {
...
static void talk_task(void *arg)
{
    ...
    int header_length = snprintf(multipart_header, sizeof(multipart_header),
        "--" SPARKY_TALK_BOUNDARY "\r\n"
        "Content-Disposition: form-data; name=\"audio\"; filename=\"capture.wav\"\r\n"
        "Content-Type: audio/wav\r\n\r\n");
    uint8_t wav_header[44];
    make_wav_header(wav_header, s_talk.pcm_bytes);

    size_t body_length = (size_t)header_length + sizeof(wav_header) + s_talk.pcm_bytes + strlen(multipart_footer);
    ...
    snprintf(content_type, sizeof(content_type), "multipart/form-data; boundary=%s", SPARKY_TALK_BOUNDARY);
    esp_http_client_set_method(client, HTTP_METHOD_POST);
    esp_http_client_set_header(client, "Content-Type", content_type);
    esp_http_client_set_header(client, "x-user-id", SPARKY_TALK_USER_ID);
    ESP_LOGI(TAG, "Sending %u-byte WAV to /talk", (unsigned)(sizeof(wav_header) + s_talk.pcm_bytes));

    esp_err_t ret = esp_http_client_open(client, (int)body_length);
    if (ret != ESP_OK || !write_all(client, multipart_header, (size_t)header_length) ||
        !write_all(client, (const char *)wav_header, sizeof(wav_header)) ||
        !write_all(client, (const char *)s_talk.pcm, s_talk.pcm_bytes) ||
        !write_all(client, multipart_footer, strlen(multipart_footer))) {
    ...
    while (esp_http_client_read(client, read_buffer, sizeof(read_buffer)) > 0) {
        /* Response bytes are collected by http_event(). */
    }
    ESP_LOGI(TAG, "HTTP %d", status);
    if (status >= 200 && status < 300) log_response();
    else ESP_LOGE(TAG, "Backend returned HTTP %d", status);
    esp_http_client_cleanup(client);
...
{
    if (s_talk.busy) return ESP_ERR_INVALID_STATE;
    if (pcm == NULL || pcm_bytes == 0 || pcm_bytes > SPARKY_TALK_MAX_WAV_BYTES - 44 || pcm_bytes % sizeof(*pcm) != 0) return ESP_ERR_INVALID_SIZE;
    s_talk.pcm = pcm;
    ...
    s_talk.response = heap_caps_malloc(SPARKY_TALK_RESPONSE_MAX_BYTES, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (s_talk.response == NULL) return ESP_ERR_NO_MEM;
    s_talk.response[0] = '\0';
    s_talk.busy = true;
    if (xTaskCreate(talk_task, "sparky_talk", 6144, NULL, 5, NULL) != pdPASS) {
        free(s_talk.response); s_talk.response = NULL; s_talk.busy = false;
        return ESP_ERR_NO_MEM;
    }
    return ESP_OK;
}

bool sparky_talk_is_busy(void) { return s_talk.busy; }
bool sparky_talk_is_complete(void) { return s_talk.complete; }
